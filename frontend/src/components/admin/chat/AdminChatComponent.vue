...
import {socket} from "@/base/service/socket.service";
export default defineComponent({
    ...
    data () {
        return {
            ...
            users: [] as User[],
            user: localStorage.getItem('user') ? JSON.parse(localStorage.getItem('user') as string) : new User(),
            message: '',
    ...
    methods: {
        async sendMessage() {
            if (this.message) {
                await this.chatService.sendMessage({
                    ...
                    toUser: this.userSelected?.id
                });

                // Gửi sự kiện
                socket.emit('chat', this.message);

                // Đặt this.message thành chuỗi trống
                this.message = '';
        ...
        async getMessages() {
            this.messageList = await this.chatService.getMessages(this.userSelected?.id);
            // xử lý bất đồng bộ
        },
